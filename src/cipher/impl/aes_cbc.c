...
/**
 * AES-CBC实现 - 对应Kotlin的AESCBC类
 * 
 * 实现双重AES-CBC加密：
 * 1. 使用key1进行第一次AES-CBC加密
 * 2. 使用key2进行第二次AES-CBC加密
 */
...
static uint8_t* aes_decrypt_cbc(const uint8_t* data, size_t data_len, 
                                const uint8_t* key, const uint8_t* iv, 
                                size_t* out_len) {
    if (data_len < 16) return NULL; // 至少需要16字节（不包含IV）
...
static char* aes_cbc_decrypt(cipher_interface_t* self, const char* hex) {
    ...
    if (!bytes || bytes_len < 32) { // 至少需要32字节（两个IV）
        safe_free(bytes);
        return NULL;
    }

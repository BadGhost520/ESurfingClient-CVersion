#!/bin/bash
filename="openwrt-sdk-21.02.7-x86-64_gcc-8.4.0_musl.Linux-x86_64.tar.xz"
...
if [[ -f "$filename" ]]; then
    ...
    wget "https://mirrors.sustech.edu.cn/openwrt/releases/21.02.7/targets/x86/64/openwrt-sdk-21.02.7-x86-64_gcc-8.4.0_musl.Linux-x86_64.tar.xz"
fi
...
tar -xf $filename
echo "Rename SDK dir"
mv openwrt-sdk-21.02.7-x86-64_gcc-8.4.0_musl.Linux-x86_64 openwrt-sdk
...
ipk_file=$(find openwrt-sdk/bin/packages/x86_64/base/ -name "esurfingclient_*.ipk" | head -1)
if [ -n "$ipk_file" ]; then
  echo "Found IPK file: $ipk_file"
  cp "$ipk_file" esurfingclient-x86_64-openwrt-21.02.7.ipk
else
